...
namespace Task3
{
    public class Task3
    {
         ...
         */
        internal static Int32 NumberOfDays( Int32 year ) => (year > 0 && ((year % 4 == 0 && year % 100 != 0) || year % 400 == 0)) ? 366 : 365;
        ...
        internal static Char Rotate2( Char orientation, Int32 cmd1, Int32 cmd2 )
        {
            ...
            Char rotate1( Char orientation, Int32 cmd ) => 
                arr[(Array.IndexOf(arr, orientation) + ((cmd == 1) ? (-1) : ((cmd == -1) ? 1 : 2)) + 4) % 4];
        ...
        internal static String AgeDescription(Int32 age)
        {
            ...
            string[] lineDescriptionTens = { "двадцать", "тридцать", "сорок", "пятьдесят", "шестьдесят" };
            string[] lineDescriptionUints = { "", "один", "два", "три", "четыре", "пять", "шесть", "семь", "восемь", "девять" };
            
            if (uints == 0 || uints >= 5) word = "лет";
            else if (uints == 1) word = "год";
            else word = "года";

            string result = lineDescriptionTens[tens - 2];
            if (uints != 0)
                result += " " + lineDescriptionUints[uints];
            result += " " + word;
